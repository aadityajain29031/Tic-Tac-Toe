...
var player2count = 0
    ...
    private fun playNow(buttonSelected: Button, currCell: Int) {
      val audio = MediaPlayer.create(this,R.raw.sound1)
        if(activeUser == 1){
            ...
            buttonSelected.setTextColor(Color.parseColor("#EC8C8C"))
            ...
            audio.start()
            buttonSelected.setTextColor(Color.parseColor("#FF0000"))
            activeUser = 1
            ...
            val checkWinner = checkWinner()
            if(checkWinner ==1){
                Handler().postDelayed(Runnable { reset() },500)
            }
        }
    ...
    private fun robot() {
       val rnd = (1..9).random()
        if (emptyCells.contains(rnd)){
            ...
            buttonSelected.text = "o"
            buttonSelected.setTextColor(Color.parseColor("#D22BB884"))
            player2.add(rnd)
            buttonSelected.isEnabled = false
    ...
    private fun checkWinner(): Int {
        val audio = MediaPlayer.create(this, R.raw.winning)
        ...
        buttonDisable()
            audio.start()
            disableReset()
            Handler().postDelayed(Runnable { audio.release() },400)
            ...
            Handler().postDelayed(Runnable { build.show() },300)
            return 1

        } else if ((player1.contains(1) && player2.contains(2) && player2.contains(3)) ||
            ...
            //to check condiotions and count player 2 wins
            player2count +=1
            audio.start()
            buttonDisable()
            disableReset()
            ...
            && emptyCells.contains(6) && emptyCells.contains(7) && emptyCells.contains(8) && emptyCells.contains(9) ){
            val build = AlertDialog.Builder(this)
            build.setTitle("Game Over")
            build.setMessage("Game draws \n\n" + " Do you want to play Again?")
            build.setPositiveButton("ok"){dialog,which->
                reset()
            }
            build.setNegativeButton("Exit"){dialog, which->
  ...
  private fun buttonDisable(){
    ...
    private fun disableReset() {
      resetBtn.isEnabled = false
        Handler().postDelayed(Runnable { resetBtn.isEnabled = true },100)
    }
}